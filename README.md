...
GlauconiteFilterer accepts a list of SMILES (in .smi format) and will output a list of compounds
that pass and fail a given filter. GlauconiteFilterer also allows users to apply chemical property filters. Additionally, GlauconiteFilterer options the automated conversion of the children from 1/2D SMILES to 3D representations (PDB and 3D-SDF) using Gypsum-DL.

![Image of Predefined_Filters](https://github.com/Jacob-Spiegel/GlauconiteFilterer/blob/main/Figures/Predefined_Filters.png)

# Using GlauconiteFilterer

GlauconiteFilterer is primarily a python based program which can be run through command-line and GUI interface. More details can be found within `/PATH/GlauconiteFilterer/tutorial/tutorial.md` or by running:

`python /PATH/GlauconiteFilterer/RunGlauconiteFilterer.py -h`

## Command-Line Interface

In a terminal with a modern Python environment run either:

`python /PATH/GlauconiteFilterer/RunGlauconiteFilterer.py -j /PATH/To/JSON_Paramerter_File.json`

or

`python /PATH/GlauconiteFilterer/RunGlauconiteFilterer.py --source_compound_file  /PATH/To/SMILES_File.smi`
...
`python /PATH/GlauconiteFilterer/RunGlauconiteFilterer_GUI.py `


# Dependencies

GlauconiteFilterer requires the following dependencies:
- RDKit
- func_timeout
- scipy
- numpy
...
GlauconiteFilterer was tested on a Python 3.7.7 environment on a Ubuntu OS. A docker image is provided for users with unsupported operating systems, such as Windows.
